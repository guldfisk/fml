...
@click.option(
    "--output-mode", type=str, help="Formatting target for output.", default="table"
)
def main(output_mode: str) -> None:
    ...
    if output_mode != "table":
        options = [mode for mode in OutputMode if output_mode in mode.value]
...
def notify(text: t.Sequence[str]) -> None:
    """
    Display os notification.
    """
    Client().notify(" ".join(text))
