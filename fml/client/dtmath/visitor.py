...
class DTMVisitor(dtmath_grammarVisitor):
    ...
    def _current_date(self) -> datetime.datetime:
        return datetime.datetime(
            year=self._now.year, month=self._now.month, day=self._now.day
        )

    def visitStart(self, ctx: dtmath_grammarParser.StartContext):
        return self.visit(ctx.operation())

    def visitAdditionOperation(
        self, ctx: dtmath_grammarParser.AdditionOperationContext
    ):
        return self.visit(ctx.operation(0)) + self.visit(ctx.operation(1))

    def visitSubtractionOperation(
        self, ctx: dtmath_grammarParser.SubtractionOperationContext
    ):
        return self.visit(ctx.operation(0)) - self.visit(ctx.operation(1))
    ...
    def visitDeltaUnitFirst(self, ctx: dtmath_grammarParser.DeltaUnitFirstContext):
        return datetime.timedelta(
            **{self.visit(ctx.delta_unit()): self.visit(ctx.any_number())}
        )
    ...
    def visitRecurringDate(self, ctx: dtmath_grammarParser.RecurringDateContext):
        return self._current_date().replace(
            day=self.visit(ctx.duo()), month=self.visit(ctx.month())
        )
    ...
    def visitNextWeekNumberDay(
        self, ctx: dtmath_grammarParser.NextWeekNumberDayContext
    ):
    ...
    def visitParenthesisOperation(
        self, ctx: dtmath_grammarParser.ParenthesisOperationContext
    ):
        return self.visit(ctx.operation())
    ...
    def visitNextRecurringDate(
        self, ctx: dtmath_grammarParser.NextRecurringDateContext
    ):
        date = self._current_date().replace(
            day=self.visit(ctx.duo()), month=self.visit(ctx.month())
        )
