...
def _get_striped_table(
    ...
    for idx, row in enumerate(rows):
        table.add_row(
            *row,
            style=Style(
                bgcolor=v.LINE_BG_COLOR_ALTERNATE if idx & 1 else v.LINE_BG_COLOR
            ),
        )
...
class _MultiItemPrinter(t.Generic[T]):
    ...
    def get_table(self, items: t.Sequence[T], **kwargs) -> Table:
        return _get_striped_table(
            self.headers,
            [
                self.format_item(item, idx=idx, **kwargs)
    ...
    def __call__(self, items: t.Union[T, t.Sequence[T]], **kwargs) -> None:
        if not isinstance(items, t.Sequence) and Context.output_mode in (
            OutputMode.TABLE,
            OutputMode.LIST,
        ):
            items = [items]
        if Context.output_mode == OutputMode.TABLE:
            ...
            for idx, item in enumerate(items):
                ...
                for key, value in zip(
                    self.headers, self.format_item(item, idx=idx, **kwargs)
                ):
                    print(
                        Text("\t")
            ...
            print(
                json.dumps(
                    ...
                    if isinstance(items, t.Sequence)
                    else dataclasses.asdict(items),
...
def print_projects(
    project: t.Sequence[models.Project], title: t.Optional[str] = None
) -> None:
    _print_striped_table(
...
def print_priorities(
    priorities: t.Sequence[models.Priority], title: t.Optional[str] = None
) -> None:
    _print_striped_table(
        ...
        [
            [
                ...
                Text(
                    priority.name,
                    style=Style(
                        color=v.PRIORITY_COLOR_MAP.get(priority.level, v.C_NEUTRAL)
                    ),
                ),
                priority.project,
                str(priority.level),
...
class TodoPrinter(_MultiItemPrinter[models.ToDo]):
    ...
    @classmethod
    def _iterate_todos(
        cls, todos: t.Sequence[models.ToDo], indent: int = 0
    ) -> t.Iterator[t.Tuple[models.ToDo, int]]:
        for todo in todos:
            yield todo, indent
    ...
    def format_item(cls, item: models.ToDo, **kwargs) -> t.Sequence[t.Any]:
        return [
            ...
            + Text(
                item.text,
                style=Style(
                    color=v.C_NEUTRAL if item.status == v.State.WAITING else v.C_WHITE
                ),
            )
            + Text(
                "".join(
                    ...
                    * (kwargs["indent"] + kwargs.get("comment_indent", 0))
                    + " - "
                    + comment
                    for comment in item.comments
                )
                if kwargs.get("show_comments", True)
                else ""
            ),
            ...
            Text(
                item.priority.name,
                style=Style(
                    color=v.PRIORITY_COLOR_MAP.get(item.priority.level, v.C_NEUTRAL)
                ),
            ),
            ", ".join(item.tags),
            item.project,
    ...
    def __call__(
        self, items: t.Union[models.ToDo, t.Sequence[models.ToDo]], **kwargs
    ) -> None:
        ...
        if Context.output_mode == OutputMode.TABLE:
            _print_striped_table(
                self.headers,
                [
                    self.format_item(item, indent=indent, **kwargs)
...
class CICheckerPrinter(_MultiItemPrinter[models.CIChecker]):
    title = None
    headers = ["Run ID", "Started", "Elapsed", "Timeout", "Status"]
...
class CIRunPrinter(_MultiItemPrinter[CIRun]):
    ...
    def name_to_link(cls, name: str) -> str:
        _diff = re.compile(r"D\d+$")
        _git_hash = re.compile("[a-z0-9]{40}$")
        if _diff.match(name):
            return "[link=https://phabricator.uniid.it/{diff}]{diff}[/link]".format(
                diff=name
            )
        if _git_hash.match(name):
            return "[link=https://github.com/unisport/unisport/commit/{hash}]{hash}[/link]".format(
                hash=name
            )
        return name
    ...
    def format_item(cls, item: CIRun, **kwargs) -> t.Sequence[t.Any]:
        return [
            ...
            Text(
                item["state"],
                style=Style(
                    color=v.CI_STATUS_COLOR_MAP.get(item["state"], v.C_NEUTRAL)
                ),
            ),
            Text(
                item["result"],
                style=Style(
                    color=v.CI_STATUS_COLOR_MAP.get(item["result"], v.C_NEUTRAL)
                ),
            ),
        ]
