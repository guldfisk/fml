...
from fml.client.cli.common import (
    AliasedGroup,
    main,
    split_text_option,
    force_option,
...
@click.option(
    ...
    "-p",
    type=str,
    help="Specify project. If not specified, use default project.",
)
...
def cancel_todo(target: t.Sequence[str], project: t.Optional[str] = None) -> None:
    ...
    """
    output.print_todo(
        Client().cancel_todo(" ".join(target), get_default_project(project))
    )
...
def toggle_todo_wait(target: t.Sequence[str], project: t.Optional[str] = None) -> None:
    ...
    """
    output.print_todo(
        Client().toggle_todo_waiting(" ".join(target), get_default_project(project))
    )
...
def finish_todo(target: t.Sequence[str], project: t.Optional[str] = None) -> None:
    ...
    """
    output.print_todo(
        Client().finish_todo(" ".join(target), get_default_project(project))
    )
...
@click.option(
    "--minimum-priority", "-m", default=None, type=str, help="Minimum priority."
)
@click.option(
    "--no-comments",
    "-c",
...
def list_todos(
    ...
    **kwargs,
    ...
    output.print_todos(
        ...
        if history
        else Client().active_todos(**kwargs),
...
def list_projects() -> None:
    ...
    List projects.
    """
    output.print_projects(Client().list_projects())
...
def create_project(
    ...
    """
    output.print_project(Client().create_project(name))
...
def modify_project_default_priority_filter(
    ...
    output.print_project(
        Client().modify_project_default_priority_filter(
            project, None if level.lower() == "none" else level
        ),
    )
