...
class StringIdentified(Base):
    ...
    def get_for_identifier(
        ...
        if not identifier:
            return cls.get_for_no_identifier(
                session, target=target, base_query=base_query
            )
        if isinstance(identifier, int):
            return base_query.filter(cls.id == identifier).scalar()
        try:
            return base_query.filter(cls.text_identifier.contains(identifier)).scalar()
    ...
    def get_list_for_identifier(
        ...
        if not identifier:
            return [
                cls.get_for_no_identifier(session, target=target, base_query=base_query)
            ]
        if isinstance(identifier, int):
            return list(base_query.filter(cls.id == identifier))

        return list(base_query.filter(cls.text_identifier.contains(identifier)))
...
class Alarm(StringIdentified):
    ...
    def cancelable(self) -> bool:
        return not self.canceled and (
            ...
            or (not self.requires_acknowledgment and self.times_notified <= 0)
        )
...
class Priority(StringIdentified):
    ...
    __table_args__ = (
        UniqueConstraint("project_id", "level"),
    ...
    def get_for_no_identifier(
        ...
        return base_query.filter(cls.is_default == True).scalar()  # noqa E712
