...
def list_alarms(
    history: bool = False, query: t.Optional[str] = None, limit: int = 10
) -> None:
    """
    List active alarms.
    """
    output.print_alarms(
        ...
        if history
        else Client().active_alarms(limit=limit, query=query)
    )
        ...
        if not force:
            ...
            if len(active_alarms) > 1:
                ...
                if not click.confirm("Cancel these alarms?", default=False):
                    return
        output.print_alarms(Client().cancel_all_alarms())
    else:
...
def acknowledge_alarms(target: str) -> None:
    ...
    target = " ".join(target)
    if target == "all":
        output.print_alarms(Client().acknowledge_all_alarms())
    else:
