...
class ClientMultiObjectContext(ClientError):
    ...
    def show(self):
        candidate_class, candidate_printer = self.type_view_map.get(
            self._message["candidate_type"], (None, None)
        )
...
class Client(object):
    ...
    def notify(self, title: str, description: str = "") -> t.Any:
        return self._make_request(
            "alarms/notify/",
            "POST",
            {
    ...
    def snooze_alarm(
        self, target: t.Union[str, int], new_target_time: datetime.datetime
    ) -> models.Alarm:
        return models.Alarm.from_remote(
            self._make_request(
                "alarms/snooze/",
    ...
    def active_alarms(
        self, limit: t.Optional[int] = None, query: t.Optional[str] = None
    ) -> t.Sequence[models.Alarm]:
        return [
            models.Alarm.from_remote(alarm)
            for alarm in self._make_request("alarms/", limit=limit, query=query)[
                "alarms"
            ]
        ]

    def alarm_history(
        self, limit: t.Optional[int] = 10, query: t.Optional[str] = None
    ) -> t.Sequence[models.Alarm]:
        return [
            models.Alarm.from_remote(alarm)
            for alarm in self._make_request(
                "alarms/history/", limit=limit, query=query
            )["alarms"]
        ]
    ...
    def cancel_all_alarms(self) -> t.Sequence[models.Alarm]:
        return [
            models.Alarm.from_remote(alarm)
            for alarm in self._make_request("alarms/cancel/", "PATCH")["alarms"]
        ]

    def acknowledge_all_alarms(self) -> t.Sequence[models.Alarm]:
        return [
            models.Alarm.from_remote(alarm)
            for alarm in self._make_request("alarms/acknowledge/all/", "PATCH")[
                "alarms"
            ]
        ]
    ...
    def list_projects(self, limit: t.Optional[int] = 25) -> t.Sequence[models.Project]:
        return [
            ...
            for project in self._make_request("project/", limit=limit)["projects"]
        ]
    ...
    def list_priorities(
        ...
        return [
            ...
            for project in self._make_request(
                "priorities/", limit=limit, project=project
            )["priorities"]
        ]
    ...
    def get_todo(
        self, todo: t.Union[str, int], project: t.Union[str, int, None] = None
    ) -> models.ToDo:
        return models.ToDo.from_remote(
            self._make_request(
                "todo/single/",
    ...
    def toggle_todo_waiting(
        self, target: str, project: t.Optional[str] = None
    ) -> models.ToDo:
        return models.ToDo.from_remote(
            self._make_request(
                "todo/toggle-wait/",
    ...
    def todo_burn_down(
        ...
        return [
            ...
            for date, active_todos in self._make_request(
                "todo/burn-down/",
                project=project,
                tag=tag,
    ...
    def todo_throughput(
        ...
        return [
            ...
            for date, throughput in self._make_request(
                "todo/throughput/",
                project=project,
                tag=tag,
                top_level_only=not all_tasks,
    ...
    def list_tags(self) -> t.Sequence[models.Tag]:
        return [
            models.Tag.from_remote(tag) for tag in self._make_request("tag/")["tags"]
        ]
    ...
    def register_dependency(
        self, parent: t.Union[int, str], child: t.Union[int, str]
    ) -> models.ToDo:
        return models.ToDo.from_remote(
            self._make_request(
                "todo/add-dependency/",
        ...
        return models.CIChecker.from_remote(
            self._make_request(
                ...
                "POST",
                {
                    ...
                    "timeout": None
                    if timeout is None
                    else timeout.strftime("%d/%m/%Y %H:%M:%S"),
                    "superseed": superseed,
                },
            )
        )
    ...
    def ci_watching(self) -> t.Sequence[models.CIChecker]:
        return [
            ...
            for checker in self._make_request(
                "ci/watching/",
            )["ci_checkers"]
        ]
